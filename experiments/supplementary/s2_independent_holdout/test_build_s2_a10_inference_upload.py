"""Unit tests for the deterministic S2 A10 inference upload builder."""
...
from build_s2_a10_inference_upload import (
...
class BuildS2InferenceUploadTests(unittest.TestCase):
    def test_label_side_filename_is_rejected(self) -> None:
        with self.assertRaises(ValueError):
            assert_no_forbidden_payload([Path("data/s2_labels.tsv.gz")])

    def test_sequence_only_filename_is_allowed(self) -> None:
    ...
    def test_archive_is_deterministic(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            source = root / "source"
            source.mkdir()
            (source / "value.txt").write_text("fixed\n", encoding="utf-8")
