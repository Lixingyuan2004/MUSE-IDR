"""Train A7: A2 local features plus gated bidirectional sequence context."""
...
from experiments.ablations.a7_bidirectional_sequence_context.model import (  # noqa: E402
...
def build_model(
    ...
    return BidirectionalSequenceContextIdrHead(
        input_size=configured_input,
        hidden_size=int(model_cfg["hidden_size"]),
        kernels=tuple(int(value) for value in model_cfg["kernels"]),
        gru_hidden_size=int(model_cfg["gru_hidden_size"]),
        gru_layers=int(model_cfg["gru_layers"]),
        dropout=float(model_cfg["dropout"]),
        sequence_dropout=float(model_cfg["sequence_dropout"]),
        sequence_gate_init=float(model_cfg["sequence_gate_init"]),
    )
...
def run_fold(
    ...
    report = {
        ...
        "context_module": "a2_local_plus_gated_bidirectional_gru",
        "context_kernels": list(model.kernels),
        "gru_hidden_size_per_direction": model.gru_hidden_size,
        "gru_layers": model.gru_layers,
        "bidirectional_sequence_context": model.sequence_context_is_bidirectional,
...
def main() -> None:
    ...
    summary = {
        ...
        "gru_hidden_size_per_direction": int(config["model"]["gru_hidden_size"]),
        "gru_layers": int(config["model"]["gru_layers"]),
        "bidirectional_sequence_context": True,
        "output_heads": 1,
        "output_definition": "classic_idr_probability_per_residue",
        "seed": seed,
        "device": str(device),
