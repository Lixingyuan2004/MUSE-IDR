"""Run a tiny end-to-end A7 fold using a verified synthetic cache."""
...
from experiments.ablations.a7_bidirectional_sequence_context.train_a7 import run_fold


def main() -> None:
    records = make_records()
    config = {
        "experiment": {"name": "a7_synthetic_smoke"},
        "model": {
            "input_size": 16,
            "hidden_size": 8,
            "kernels": [3, 5],
    ...
    with tempfile.TemporaryDirectory() as directory:
        ...
        if report["output_heads"] != 1:
            raise RuntimeError("A7 end-to-end smoke created more than one output head")
        if report["metrics"]["micro_roc_auc"] is None:
            raise RuntimeError("A7 end-to-end smoke did not produce ROC-AUC")
        if not report["bidirectional_sequence_context"]:
            raise RuntimeError("A7 end-to-end smoke did not use bidirectional context")
        ...
        if not all(path.is_file() for path in required):
            raise RuntimeError("A7 end-to-end smoke did not write all fold artifacts")
        ...
        json.dumps(
            {
                "status": "pass",
                "fold_training": True,
                "bidirectional_sequence_context_trained": True,
