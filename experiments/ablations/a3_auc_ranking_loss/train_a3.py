"""Train A3: A2 multiscale context with sampled pairwise AUC loss."""
...
def train_epoch(
    ...
    for batch in loader:
        ...
        residue_mask = batch["residue_mask"].to(device, non_blocking=True)
        known = residue_mask & (labels != float(unknown_label))
        if not torch.any(known):
            continue

        logits = model(features, residue_mask)
        losses = combined_bce_auc_loss(
            logits,
            labels,
            known,
        ...
        weighted_bce += float(losses.bce.detach().cpu()) * losses.known_residues
        known_residues += losses.known_residues
        if losses.ranking_pairs:
            weighted_ranking += (
                float(losses.ranking.detach().cpu()) * losses.ranking_pairs
            )
            ranking_pairs += losses.ranking_pairs

    if known_residues == 0:
        raise RuntimeError("training epoch contains no known labels")
    if rank_weight > 0 and ranking_pairs == 0:
        raise RuntimeError("AUC ranking loss received no positive-negative pairs")

    mean_bce = weighted_bce / known_residues
    mean_ranking = weighted_ranking / ranking_pairs if ranking_pairs else 0.0
    return {
        "train_loss": mean_bce + rank_weight * mean_ranking,
        "train_bce_loss": mean_bce,
        "train_rank_loss": mean_ranking,
        "train_known_residues": known_residues,
        "train_rank_pairs": ranking_pairs,
    }
...
def run_fold(
    ...
    train_loader = make_loader(
        train_records, cache, batch_size, num_workers, unknown_label, True, seed
    )
    val_loader = make_loader(
        val_records, cache, batch_size, num_workers, unknown_label, False, seed
    )
    ...
    for epoch in range(1, int(train_cfg["epochs"]) + 1):
        train_metrics = train_epoch(
            ...
            float(loss_cfg["rank_weight"]),
            float(loss_cfg["rank_margin"]),
            int(loss_cfg["max_pairs_per_batch"]),
        )
    ...
    report = {
        ...
        "loss_function": str(loss_cfg["name"]),
...
def main() -> None:
    ...
    summary = {
        ...
        "loss_function": str(config["loss"]["name"]),
