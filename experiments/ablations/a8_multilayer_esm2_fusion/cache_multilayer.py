"""Cache selected frozen ESM2 layers for every residue without reading labels."""
...
def load_sequence_manifest(path: Path) -> list[SequenceRecord]:
    ...
    with path.open("r", encoding="utf-8") as handle:
        for line_number, raw_line in enumerate(handle, start=1):
            ...
            if forbidden:
                raise ValueError(
                    "cache input must be label-free; "
                    f"found {sorted(forbidden)} at line {line_number}"
                )
...
def cached_entry_if_valid(
    ...
    try:
        ...
        if embedding.shape != expected_shape:
            return None
        if str(embedding.dtype) != str(metadata["dtype"]):
            return None
...
def encode_record(
    ...
    weighted_hidden = np.zeros(
        (len(record.sequence), len(layer_indices), hidden_size), dtype=np.float32
    )
    ...
    for batch_start in range(0, len(spans), window_batch_size):
        ...
        residue_mask = encoded["attention_mask"].bool() & ~encoded[
            "special_tokens_mask"
        ].bool()
        ...
        selected = torch.stack(
            [hidden_states[layer_index] for layer_index in layer_indices], dim=2
        ).float().cpu()

        for index, (start, end) in enumerate(batch_spans):
            residue_hidden = selected[index][residue_mask[index]]
            expected_shape = (end - start, len(layer_indices), hidden_size)
            if residue_hidden.shape != expected_shape:
                raise RuntimeError(
                    f"{record.protein_id}[{start}:{end}] token alignment failure: "
                    f"{tuple(residue_hidden.shape)} != {expected_shape}"
                )
            weights = center_merge_weights(end - start)
            weighted_hidden[start:end] += residue_hidden.numpy() * weights[:, None, None]
            weight_sum[start:end] += weights

    if np.any(weight_sum <= 0):
        raise RuntimeError(f"uncovered residues while caching {record.protein_id}")
    merged = weighted_hidden / weight_sum[:, None, None]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config", type=Path, default=Path(__file__).with_name("cache_config.yaml")
    )
...
def main() -> None:
    ...
    if storage_dtype != np.float16:
        raise ValueError("A8 cache requires float16 storage")
    ...
    if layer_indices[0] < 1 or layer_indices[-1] > num_hidden_layers:
        raise ValueError(
            f"selected layers {layer_indices} outside transformer layers 1..{num_hidden_layers}"
        )
    if device.type == "cuda":
    ...
    for index, record in enumerate(records, start=1):
        ...
        if reused:
            ...
            metadata = {
                **expected,
                "fold": record.fold,
                "hidden_size": int(representation.shape[2]),
            ...
            json.dumps(
                {
                    ...
                    "source": "cache" if reused else "generated",
                },
                separators=(",", ":"),
            ),
            flush=True,
