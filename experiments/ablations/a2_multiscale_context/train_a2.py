"""Train A2: multiscale context over a verified frozen ESM2 residue cache."""
...
from experiments.ablations.a2_multiscale_context.data import (  # noqa: E402
    CachedProteinDataset,
    pad_cached_proteins,
)
...
def make_loader(
    ...
    return DataLoader(
        CachedProteinDataset(records, cache),
        batch_size=batch_size,
        shuffle=shuffle,
        generator=generator,
        num_workers=num_workers,
        pin_memory=torch.cuda.is_available(),
        collate_fn=lambda examples: pad_cached_proteins(examples, unknown_label),
    )


def build_model(config: dict[str, Any], cache: CacheBundle) -> MultiscaleContextIdrHead:
...
def train_epoch(
    ...
    for batch in loader:
        ...
        loss = F.binary_cross_entropy_with_logits(
            logits[known], labels[known], pos_weight=positive_weight
        )
...
def predict_records(
    ...
    for batch in loader:
        ...
        residue_mask = batch["residue_mask"].to(device, non_blocking=True)
        probabilities = torch.sigmoid(model(features, residue_mask)).cpu()
        for index, protein_id in enumerate(batch["protein_ids"]):
            length = int(batch["lengths"][index])
            predictions[str(protein_id)] = (
                probabilities[index, :length].numpy().astype(np.float32, copy=False)
            )
    return predictions
